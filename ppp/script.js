...
let channel_title = 'ppp-6mhtqsls3uy';
...
function createThumbnail(item) {
    if (item.class == 'Image' && !uniqueUrls.has(item.image.display.url)) {
        ...
        let thumb_tx = document.createElement('span');
        thumb_tx.classList.add('title');
        thumb_tx.innerHTML = `${item.title}`;
